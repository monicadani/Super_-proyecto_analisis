...
def pendent_pair(Vprime, V, S, f, params=None):
    ...
    for i in range(n-1):
        ...
        for j in range(n):
            if used[j]:
                continue
            keys[j] = submodular(Wi + S[Vprime[j]], V, params) - submodular(S[Vprime[j]], V, params)
...
if __name__=='__main__':
    print('Tests to come :)')
    optimal_set([[2,3],[1,3,2],[1,2]], None)
